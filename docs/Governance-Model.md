...
This command runs info-doc lint, style lint, Angular lint, library tests with coverage, library build, and application build. It writes the latest report to `reports/quality/latest.md`.
...
Recent editor formatting work:

- Multi-paragraph and select-all text alignment now applies to live selected blocks.
- Header state is reflected in the toolbar dropdown.
- Selecting `Normal` converts selected H1-H6 blocks back to paragraphs.
- Re-selecting the already active heading in the native dropdown does not emit a browser `change` event; use `Normal` to remove heading formatting.
